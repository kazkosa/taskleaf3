      ...
      <transition-group class="project-member-list" name="items" tag="ul">
        ...
        <li v-for="item in members" v-bind:key="item.id" class="project-member-list__member">
          ...
          <div class="member-item">
            ...
            <div v-if="getAuthorityList(item).length && currentUserRoleInThisPj < 2" class="member-item__role">
            ...
            <div v-else class="member-item__role" :class="(currentUserRoleInThisPj==2 && item.user_id == currentUser.id)? 'noedit2': 'noedit'">{{ item.role.charAt(0).toUpperCase() + item.role.slice(1)}}</div> 

            <div v-if="getAuthorityList(item).length && ( currentUserRoleInThisPj < 2 || (item.user_id == currentUser.id) )" class="member-item__remove xinvisible" @click="openFormDeleteProjectMember(item)">
              <span></span>
            </div>
