  ...
  <transition name="modal">
    <div v-if="isShow" id="modal_m1-01" class="modal js-modal" key="modal1">
      ...
      <div class="modal__content modal__content-project-form">
        ...
        <h4 class="modal_title"><span>Add Workspace Member</span></h4>
...
export default {
  ...
  computed: {
    ...
    getAuthorityList: function() {
      ...
      target_authority_list = authority_list.filter( function(item) {
        return  item.id >= _this.currentUserRoleInThisWs
      })
  ...
  methods: {
    ...
    submitAddMembers: function() {
      ...
      axios.put('/api/workspaces/' + this.workspaceId + '/update_members/', params)
      .then((res) => {
        this.modalClose()
        this.$emit('update-workspace-member', this.workspaceId)
      }, (error) => {
        console.log(error);
      });
    },
