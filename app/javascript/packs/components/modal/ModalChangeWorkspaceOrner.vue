  ...
  <transition name="modal">
    <div v-if="isShow" id="modal_m1-01" class="modal js-modal" key="modal1">
      ...
      <div class="modal__content modal__content-project-form">
        ...
        <h4 class="modal_title"><span>Change the owner of workspace "{{workspace.name}}"</span></h4>
        <p class="modal_message">Are you sure you want to change the owner from {{currentOrner.name}} to {{user.name}}?</p>
        <button @click="changeWorkspaceOrner" class="btnSubmit">Yes</button>
...
export default {
  ...
  methods: {
    ...
    changeWorkspaceOrner: function() {
      axios.put('/api/workspace_members/' + this.user.id, { workspace_member: { role: 0 } })
      .then((res) => {
        axios.put('/api/workspace_members/' + this.currentOrner.id, { workspace_member: { role: 1 } })
          .then((res) => {
